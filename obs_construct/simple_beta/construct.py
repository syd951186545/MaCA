    ...
    def __get_data_obs(self, detector_data_obs_list, fighter_data_obs_list, joint_data_obs_dict):
        ...
        for x in range(self.fighter_num):
            if fighter_data_obs_list[x]['alive']:
                # 增加空间方位相对值，以及上一时刻自身运动方位
                fighter_data[x, 0] = fighter_data_obs_list[x]['pos_x']/self.battlefield_size_x
                fighter_data[x, 1] = fighter_data_obs_list[x]['pos_y']/self.battlefield_size_y
                fighter_data[x, 2] = fighter_data_obs_list[x]['course']
                fighter_data[x, 3] = fighter_data_obs_list[x]["last_action"]['course']
                fighter_data[x, 4] = fighter_data_obs_list[x]['l_missile_left']
                fighter_data[x, 5] = fighter_data_obs_list[x]['s_missile_left']
